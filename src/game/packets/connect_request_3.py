from game.enums import PACKET_TYPE, GAME_MODE, FONT, COLOR_CYCLE
...
class BATTLE_ROYALE_ACTION(Packet):
    ...
    def write(self) -> bytes:
        ...
        stream: JavaDataOutputStream = (
            ...
            .write_utf("")
            .write_byte(self.client.server_data.profile_visibility.value)
            .write_bool(self.go_mayhem)
            .write_short(self.client.wardrobe.skin.value)
            .write_byte(0xFF)
            .write_utf(self.client.server_data.name)
            .write_int(0x00000000)
            .write_byte(len(self.client.server_data.name))
        )

        for _ in range(len(self.client.server_data.name)):
            stream.write_byte(0xFF)

        (
            stream.write_byte(0xFF)
            .write_int(self.client.server_data.rng_token2)
            .write_utf("")
        ...
        self.stream = self.client.app.wire.verify(stream.get_stream())
