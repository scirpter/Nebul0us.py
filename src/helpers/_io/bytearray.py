...
class ByteArray:
    ...
    def read_from_behind(self, n: int) -> bytes:
        array_length = self._stream.getbuffer().nbytes
        if n > array_length:
            raise ValueError("Not enough data in stream")
        self._stream.seek(array_length - n)
